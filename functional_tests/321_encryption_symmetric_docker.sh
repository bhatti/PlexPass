...
docker run -e DEVICE_PEPPER_KEY=$DEVICE_PEPPER_KEY -e DATA_DIR=/data -v $PARENT_DIR/PlexPassData:/data -v $CWD:/files plexpass -j true --master-username frank --master-password Cru5h_rfIt:v_Bk symmetric-decrypt --secret-key $prv --in-path /files/enc_accounts.csv --out-path /files/enc_accounts.csv
